...
| 1 | Observation aggregation | Multi-sensor time sync, configurable downsampling, state interface broadcaster | State interface broadcaster with time synchronization and able to sync other sensor data like camera | Separate synchronization nodes using message_filters |
...
| 1 | Action post-processing | Denormalization utilities, safety constraints, chunk interpolation | Built-in utilities in controller framework for action transformation | Post-processing in separate nodes before forwarding commands to controller (or) pluginlib approach |
